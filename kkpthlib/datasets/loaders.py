...
def get_kkpthlib_dataset_dir(dirname=None):
    ...
    if not os.path.exists(subdir):
        logger.info("KKPTHLIB_DATASETS subdirectory {} not found, creating".format(subdir))
        os.mkdir(subdir)
    return subdir
...
def check_fetch_mnist():
    mnist_dir = get_kkpthlib_dataset_dir("mnist")
...
def check_fetch_fruitspeech():
    ...
    partial_path = get_kkpthlib_dataset_dir("fruitspeech")
...
def check_fetch_norvig_words():
    partial_path = get_kkpthlib_dataset_dir("norvig_words")
