...
def random_split(mel, axis_splits_str, axis_splits_offset, split_n_mels):
    # mel expected as batch, time, freq
    m = mel
    axis_splits = axis_splits_str

    m_in = m
    if axis_splits is not None and axis_splits_offset is not None:
        axis_split_list = [int(str(axis_splits)[i]) for i in range(len(str(axis_splits)))]
        ...
        axis1_m = [2 for a in str(axis_splits)[axis_splits_offset:] if a == "1"]
        axis2_m = [2 for a in str(axis_splits)[axis_splits_offset:] if a == "2"]
        ...
        subsampled = [patchify(m_t[i, :, :, 0], (axis1_m, axis2_m)) for i in range(m_t.shape[0])]

        # 50% grid points, 50% random
        grid_points = [g for g in np.unique(np.linspace(0, subsampled[0].shape[1] - 1,  split_n_mels // 2).astype(int))]
        r_ind = np.arange(subsampled[0].shape[1])
        r_ind = [p for p in r_ind if p not in grid_points]
        random_state = np.random.RandomState(1213)
        random_state.shuffle(r_ind)
        r_ind_sub = np.sort(grid_points + r_ind[:(split_n_mels - len(grid_points))])
        ...
        partial_sub = [s[::axis1_m, r_ind_sub, 0] for s in subsampled]
        """
        not currently used, would slightly more even the frequencies
        ...
        def mkel(ps):
            final_sub = []
            for _i in range(ps.shape[0]):
                pairs_0 = np.arange(ps.shape[1])
                # want to get indices up to but not including 8
                pairs_1 = np.linspace(0, ps.shape[2], num=ps.shape[1], endpoint=False).astype(int)
                pairs = list(zip(pairs_0, pairs_1))
                frame_sub = []
                for _j in range(len(pairs)):
                    frame_sub.append(ps[_i, pairs[_j][0], pairs[_j][1]])
                final_sub.append(np.array(frame_sub)[None])
            m_in = np.concatenate(final_sub)
            # final result is time, freq
            return m_in
        final_sub = [mkel(ps) for ps in partial_sub]
        """
        m_in = np.concatenate([ps[:, :, 0][None] for ps in partial_sub])
        # returns as batch, time, freq again
        return m_in
...
if __name__ == "__main__":
    ...
    if not os.path.exists(mean_std_path):
        ...
        while count < n_sample_estimate:
            ...
            cond_seq_data_batch, cond_seq_mask, data_batch, data_mask = speech.format_minibatch(els,
                                                                                                quantize_to_n_bins=None)
            x_t = data_batch
            # fix axis offset to 0 for now
            # input_size_at_depth should be downsampled_time, downsampled_freq
            x_in = random_split(x_t, axis_splits_str, 0, input_size_at_depth[1])
            # add a trailing 1 dim
            x_in = x_in[..., None]

            x_mask_t = data_mask[..., None, None] + 0. * data_batch[..., None]
            x_mask_t = x_mask_t[..., 0]
            x_mask_in = random_split(x_mask_t, axis_splits_str, 0, input_size_at_depth[1])
            # add a trailing 1 dim
            x_mask_in = x_mask_in[..., None]
    ...
    def loop(itr, extras, stateful_args):
        ...
        for _step in range(step_count):
            ...
            x_mask_t = data_mask[..., None, None] + 0. * data_batch[..., None]
            x_mask_t = x_mask_t[..., 0]
            x_mask_in_np = random_split(x_mask_t, axis_splits_str, 0, input_size_at_depth[1])
            # add a trailing 1 dim
            x_mask_in_np = x_mask_in_np[..., None]
