...
from relative_transformer_music_model import get_hparams
from relative_transformer_music_model import build_model
...
true_piano_roll = _[:16]
#true_piano_roll = np.zeros((16, hp.batch_size, 1))
sampling_random_state = np.random.RandomState(13)
#sampling_random_state = np.random.RandomState(2142)
steps = 1000 - len(true_piano_roll) - 1
steps = 256
assert steps % 4 == 0
...
with torch.no_grad():
    for t in range(steps):
        ...
        # todo, construck masks and clocks dynamically for arbitrary length generation
        ...
        if len(this_piano_roll) > cut:
            this_piano_roll = this_piano_roll[-cut:]
            this_mask = this_mask[-cut:]
            this_clocks = [torch.Tensor(tc[-cut:]).to(hp.use_device) for tc in this_clocks]

        linear_out, past = m(this_piano_roll, this_mask, this_clocks)
        reduced_probs = softmax_np(linear_out.cpu().data.numpy())
