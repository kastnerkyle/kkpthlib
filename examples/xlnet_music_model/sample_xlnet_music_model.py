...
for k in range(np_data.shape[1]):
    ...
    np_data[blank_spots, k, 0] = 67 #max(flat_measure_corpus.pitch_dictionary.word2idx[99].keys())
...
while not all(finished_sampling):
    ...
    for k in range(np_data.shape[1]):
        ...
        context_sequence = [flat_measure_corpus.pitch_dictionary.idx2word[c] for c in np_data[:, k, 0]]
        if len(filled_sequences[k]) == 0:
            # true data 0, blanked out data 1
            filled_sequences[k].append([flat_measure_corpus.pitch_dictionary.idx2word[c] for c in true_data[:, k, 0]])
        ...
        new_context_sequence = [flat_measure_corpus.pitch_dictionary.idx2word[c] for c in np_data[:, k, 0]]
