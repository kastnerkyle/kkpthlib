...
r, c_l_s, l_s, s1, s2 = model(data_batch)
...
for i in range(r.shape[0]):
    ...
    axarr[1].imshow((r[i, 0].cpu().data.numpy() > 0.5), cmap="gray")
    axarr[2].imshow(s2.reshape((50, 32, -1))[i].cpu().data.numpy())
    plt.savefig("sampled/samp{}.png".format(i))
    plt.close()
print("finished")
